//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Polyteks.Katman.DAL.Concrete.EntityFramework
{
    using System;
    using System.Collections.Generic;
    
    public partial class ProjePlanlananToplanti
    {
        public int ProjePlanlananToplantiID { get; set; }
        public Nullable<int> ProjeID { get; set; }
        public Nullable<System.DateTime> PlanlananTarih { get; set; }
        public string PlanlananToplantiKonusu { get; set; }
        public string PlanlananYer { get; set; }
        public string PlanlananToplantiAciklama { get; set; }
        public Nullable<int> ProjeToplantiID { get; set; }
        public string AktifPasif { get; set; }
        public string IptalEdenKodu { get; set; }
        public Nullable<System.DateTime> IptalTarihi { get; set; }
        public string Iptalmi { get; set; }
        public Nullable<int> MesajID { get; set; }
    }
}
